...
export const buildPortfolioStats = ({
  ...
  sortedTransactions.forEach((tx) => {
    ...
    holding.시장 = activeMarket;

    if (tx.구분 === "매수") {
      cashBalance -= totalKrw;
      holding.보유량 += quantity;
      if (isForeign) {
        holding.총매입금액달러 += quantity * price + fee + tax;
      }
      holding.총매입금액원화 += totalKrw;
    } else {
      cashBalance += totalKrw;
      const avgPriceKrw =
        holding.보유량 > 0 ? holding.총매입금액원화 / holding.보유량 : 0;
      const realizedKrw = totalKrw - quantity * avgPriceKrw;
      totalRealizedProfit += realizedKrw;
      holding.실현손익원화 += realizedKrw;

      if (isForeign) {
        const avgPriceUsd =
          holding.보유량 > 0 ? holding.총매입금액달러 / holding.보유량 : 0;
  ...
  const holdingList = Object.values(holdingMap)
    ...
    .map((holding) => {
      ...
      const evaluationAmount = isForeign
        ? holding.보유량 * currentPrice * exchangeRate
        : holding.보유량 * currentPrice;
      ...
      return {
        ...
        보유량: holding.보유량,
        섹터: sectorName,
        평균단가: holding.보유량 > 0 ? holding.총매입금액원화 / holding.보유량 : 0,
        평균단가외국:
          holding.보유량 > 0 ? holding.총매입금액달러 / holding.보유량 : 0,
        현재가: currentPrice,
    ...
    allDatesArray.forEach((date) => {
      ...
      transactions
        ...
        .forEach((tx) => {
          ...
          runningHoldings[name].시장 = activeMarket;

          if (tx.구분 === "매수") {
            runningCash -= totalKrw;
            runningHoldings[name].qty += quantity;
            runningHoldings[name].totalCostKrw += totalKrw;
          } else {
            runningCash += totalKrw;
            const avgKrw =
      ...
      let dayEvaluation = 0;
      Object.keys(runningHoldings).forEach((name) => {
        if (runningHoldings[name].qty > 0) {
      ...
      const dayAsset = dayEvaluation + runningCash;
      ...
      dailyList.push({
        ...
        일수익률:
          runningInvestment > 0
            ...
            : "0.00%",
        누적원금: Math.round(runningInvestment),
      });
    });
  ...
  const monthlyList = Object.keys(monthlyMap)
    ...
    .map((month) => {
      ...
      return {
        ...
        수익률:
          monthData.누적원금 > 0
            ? ((monthData.일손익 / monthData.누적원금) * 100).toFixed(2) + "%"
            : "0.00%",
      };
    });
