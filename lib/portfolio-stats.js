...
const buildDailyCashFlowMap = (cashFlows) => {
  const map = {};

  cashFlows.forEach((cash) => {
    const amount = parseCleanNum(cash.금액);
    const signedAmount = cash.구분 === "입금" ? amount : -amount;
    map[cash.날짜] = (map[cash.날짜] || 0) + signedAmount;
  });
...
export const buildPortfolioStats = ({
  ...
  const dailyCashFlowMap = buildDailyCashFlowMap(cashFlows);
  const dailyList = [];
  ...
  if (rawDates.length > 0) {
    ...
    allDatesArray.forEach((date) => {
      ...
      dailyList.push({
        ...
        당일현금흐름: Math.round(dailyCashFlowMap[date] || 0),
  ...
  dailyList.forEach((daily) => {
    ...
    if (!monthlyMap[monthKey]) {
      ...
      monthlyMap[monthKey].월간손익 = daily.평가손익;
