...
const getTradeAmounts = ({ tx, quantity, price, fee, tax, isForeign, exchangeRate }) => {
  const savedTotalKrw = parseCleanNum(tx.원화계산총액);
  const savedExchangeRate = parseCleanNum(tx.거래환율);
  const tradeExchangeRate = isForeign ? savedExchangeRate || exchangeRate : 1;
  const principalKrw = isForeign ? quantity * price * tradeExchangeRate : quantity * price;
  const feeTaxKrw = isForeign ? (fee + tax) * tradeExchangeRate : fee + tax;

  if (savedTotalKrw > 0) {
    return tx.구분 === "매수"
      ? {
          tradePrincipalKrw: Math.max(0, savedTotalKrw - feeTaxKrw),
          feeTaxKrw,
          buyCashOutKrw: savedTotalKrw,
          sellCashInKrw: 0,
        }
      : {
          tradePrincipalKrw: principalKrw,
          feeTaxKrw,
          buyCashOutKrw: 0,
          sellCashInKrw: savedTotalKrw,
        };
  }
  ...
  return {
    tradePrincipalKrw: principalKrw,
    feeTaxKrw,
    buyCashOutKrw: principalKrw + feeTaxKrw,
    sellCashInKrw: principalKrw - feeTaxKrw,
  };
};
...
export const buildPortfolioStats = ({
  ...
  sortedTransactions.forEach((tx) => {
    ...
    const { tradePrincipalKrw, buyCashOutKrw, sellCashInKrw } = getTradeAmounts({
      tx,
      quantity,
      price,
      fee,
  ...
  if (rawDates.length > 0) {
    ...
    allDatesArray.forEach((date) => {
      ...
      Object.entries(runningHoldings).forEach(([name, holding]) => {
        if (holding.qty <= 0) return;
        previousHoldingState[name] = {
          qty: holding.qty,
          price: holding.lastValuationPrice || holding.currentPrice || 0,
          isForeign: holding.isForeign,
        };
      });
      ...
      transactions
        ...
        .forEach((tx) => {
          ...
          if (tx.구분 === "매수") {
            ...
            if (!dayBuysByName[name]) {
              dayBuysByName[name] = { qty: 0, costKrw: 0 };
            }
            dayBuysByName[name].qty += quantity;
            dayBuysByName[name].costKrw += tradePrincipalKrw;
          } else {
      ...
      let marketOnlyBaseEvaluation = 0;
      ...
      Object.entries(runningHoldings).forEach(([name, holding]) => {
        ...
        const valuationPriceKrw = holding.isForeign
          ? dynamicPrice * exchangeRate
          : dynamicPrice;
        const previous = previousHoldingState[name];
        const previousPriceKrw = previous
          ? previous.isForeign
            ? previous.price * exchangeRate
            : previous.price
          : 0;
        const existingQty = previous ? Math.min(holding.qty, previous.qty) : 0;

        if (existingQty > 0 && previousPriceKrw > 0) {
          marketOnlyDayProfit += existingQty * (valuationPriceKrw - previousPriceKrw);
          marketOnlyBaseEvaluation += existingQty * previousPriceKrw;
        }
        ...
        if (buyInfo?.qty > 0) {
          const boughtQtyStillHeld = Math.min(
            buyInfo.qty,
            Math.max(0, holding.qty - existingQty),
          );
          const avgBuyPriceKrw = buyInfo.costKrw / buyInfo.qty;
          if (boughtQtyStillHeld > 0 && avgBuyPriceKrw > 0) {
            marketOnlyDayProfit += boughtQtyStillHeld * (valuationPriceKrw - avgBuyPriceKrw);
            marketOnlyBaseEvaluation += boughtQtyStillHeld * avgBuyPriceKrw;
          }
        }
      });
      ...
      const dayProfit = marketOnlyDayProfit;
      const dayProfitRate =
        marketOnlyBaseEvaluation > 0 ? (dayProfit / marketOnlyBaseEvaluation) * 100 : 0;
