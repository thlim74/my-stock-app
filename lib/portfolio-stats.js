...
export const buildPortfolioStats = ({
  ...
  if (rawDates.length > 0) {
    ...
    allDatesArray.forEach((date) => {
      ...
      let hasAnyHistoricalPrice = false;
      ...
      const isHoliday = date !== today && hasActiveHolding && !hasAnyHistoricalPrice;
      const computePreviousClose = (historyByDate, targetDate) => {
      ...
      // 오늘은 "전일종가 대비 현재(애프터 포함) 평가손익" 기준으로 통일한다.
      if (date === today) {
        let marketOnlyDayProfit = 0;
        Object.values(runningHoldings).forEach((holding) => {
          if (holding.qty <= 0) return;
          const historyByDate = dailyPriceHistoryMap[holding.ticker] || {};
          const prevClose = computePreviousClose(historyByDate, today);
