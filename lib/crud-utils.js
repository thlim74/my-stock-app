...
export const buildTransactionPayload = ({
  ...
  const currentTicker = masterMatch
    ...
    : newTx.티커
      ? newTx.티커.trim()
      : "999999";
  const market = masterMatch ? masterMatch.시장 : inferMarketFromTicker(currentTicker);
  const isForeign = isForeignMarket(market, currentTicker);
  const tradeExchangeRate = isForeign
    ? parseCleanNum(newTx.거래환율) || Number(exchangeRate) || 0
    : 1;
  const signedTotal = newTx.구분 === "매수"
    ? quantity * price + fee + tax
    : quantity * price - fee - tax;
  const totalKrw = isForeign ? signedTotal * tradeExchangeRate : signedTotal;
