...
export default function IntegratedPortfolioManagementSystem() {
  ...
  const summary = useMemo(() => {
    const netInv = 45137473; // 이미지 고정 원금
    const currentPrices = dailyPrices[0];
    const prevPrices = dailyPrices[1];

    const holdings = holdingsData.map((h) => {
      const cur = currentPrices[h.ticker];
      const pre = prevPrices[h.ticker];
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] p-6 text-slate-800">
      ...
      <div className="bg-white rounded-[40px] shadow-2xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
        <div className="flex bg-slate-50/50 p-2 overflow-x-auto scrollbar-hide border-b border-slate-100">
          {tabs.map((t) => (
            <button
              key={t}
        ...
        <div className="p-8">
              ...
              <table className="w-full text-left font-bold border-collapse">
                <thead className="text-slate-400 border-b">
                  <tr>
                    ...
                    <th className="pb-4">월초평가액</th>
                    <th className="pb-4">월말평가액</th>
          ...
          {activeTab === "거래관리" && (
            <div>
              <div className="bg-slate-50 p-8 rounded-[30px] border border-slate-100 mb-8 grid grid-cols-5 gap-4 items-end">
                ...
                <button className="bg-[#1e293b] text-white p-3.5 rounded-xl font-black text-xs shadow-xl shadow-slate-200">
                  거래 저장
                </button>
              </div>
          ...
          {activeTab === "종목마스터" && (
            <div className="overflow-x-auto text-[11px] font-bold">
              <table className="w-full text-left border-collapse">
                <thead className="text-slate-400 border-b font-black uppercase">
                  <tr>
                    <th className="pb-4">티커</th>
                    <th className="pb-4">종목명</th>
                    <th className="pb-4">통화</th>
                    <th className="pb-4">상장시장</th>
                    <th className="pb-4 text-right px-6">상태</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
            ...
            <div>
              ...
              <table className="w-full text-left text-[11px] font-bold">
                <thead className="text-slate-400 border-b uppercase">
                  <tr>
                    <th className="pb-4">수집일자</th>
                    <th className="pb-4">티커</th>
                    <th className="pb-4 text-right px-6">종가</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {dailyPrices.map((row, i) =>
                    Object.keys(row)
                      .filter((key) => key !== "date")
                      .map((ticker, j) => (
                        <tr key={`${i}-${j}`}>
                          ...
                          <td className="font-black">{ticker}</td>
                          <td className="text-right px-6 text-blue-600 font-black">
                            {row[ticker].toLocaleString()}
                          </td>
                        </tr>
                      )),
                  )}
