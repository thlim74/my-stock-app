  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-6 gap-3 sm:gap-4 mb-6">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm text-center relative">
        <p className="text-[10px] font-black text-slate-400 mb-1">총자산액 (DoD 트래커)</p>
        <p className="text-lg sm:text-xl font-black text-slate-800">
          ₩{formatNum(stats.totalAsset)}
        </p>
        <span
          className={`text-[10px] font-black block mt-0.5 ${
            stats.assetChangeDoD >= 0 ? "text-rose-500" : "text-blue-500"
          }`}
        >
          {stats.assetChangeDoD >= 0 ? "▲" : "▼"} ₩{formatNum(Math.abs(stats.assetChangeDoD))} (
          {stats.assetChangeRateDoD.toFixed(2)}%)
        </span>
      </div>

      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm text-center">
        <p className="text-[10px] font-black text-slate-400 mb-1">총평가손익률</p>
        <p
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm text-center">
        <p className="text-[10px] font-black text-slate-400 mb-1">누적 확정 실현손익</p>
        <p
          className={`text-lg sm:text-xl font-black ${
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm text-center">
        <p className="text-[10px] font-black text-slate-400 mb-1">실시간 예수금 잔고</p>
